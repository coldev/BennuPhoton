...
type Bullet2D   //paquete personalizado
    int tipo_paquete = 2;     
	int x;	
	int y; 
	int angle;
...
type flamethrower2D   //paquete personalizado
    int tipo_paquete = 3;     
	int x;	
	int y; 
	int angle;
...
global
   ...
   Bullet2D   Balas_Posicion[total_conectados];
   flamethrower2D Llamas_Posicion[total_conectados];
   int contador_balas;
   int contador_llamas;
...
tamano_bullet2d = sizeof(bullet2d);
tamano_llama2d = sizeof( flamethrower2D);
...
begin
  ...
  gerolo_network(codigo);
  brazo_network(codigo );
...
//================  READ PACKETS ==================  	 
     while (  (paquete_recibido = PHOTON_PACK_GET()  )    > 0)     
	  ...
	  if (generico.tipo_paquete == 2 AND paquete_recibido.player_id <> photon_player_id)	//bullet2d  
	    contador_balas++;
		
		if (contador_balas >= total_conectados)
		contador_balas=0;
  		end
		
		//crea un cliente si no existe....
         memcopy(&Balas_Posicion[contador_balas],
		paquete_recibido.data, paquete_recibido.data_length);
	 ...
	 if (generico.tipo_paquete == 3 AND paquete_recibido.player_id <> photon_player_id)	//llamas2d  
	    contador_llamas++;
		
		if (contador_llamas >= total_conectados)
		contador_llamas=0;
  		end
		
		//crea un cliente si no existe....
         memcopy(&Llamas_Posicion[contador_llamas],
		paquete_recibido.data, paquete_recibido.data_length);
...
begin
  ...
  //====================   ARMA PAQUETE PARA ENVIAR A OTROS CLIENTES =========================
 	...
 	//photon_pack_send(& paquete,false);
