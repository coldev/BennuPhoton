...
Begin
 ...
 //==========================================  
  ...
  write(0,100,10,0,"  Photon. (Room List Example) ");
   ...
   loop     
	  ...
	  while (   (  evento_red = PHOTON_EVENT_GET()  )     > 0)
		 ...
		 elseif (evento == PHOTON_EVENT_JOINED )
			 ...
			 create_list_room_event();//only available in join lobby event
...
process create_list_room_event()
private
  int  i, totalrooms;  
...
begin
  totalrooms= photon_room_list( ); //list match games
  
  
  write(0,50,100,0,"Room Total: "+itoa(totalrooms));   
  
  
  for (i=0;i<totalrooms;i++)
    room =  photon_room_get( i); 
  
    if (room>0)   
     write(0,50,110+(i*10),0,
	 room.name+ "  (" +
