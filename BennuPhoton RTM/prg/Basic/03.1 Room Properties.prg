...
tamano_posicion2D = sizeof(Posicion2D);
  ...
  set_fps (200,0); //max.  frames        
   ...
   photon_player_max = 20;  //20 jugadores maximo 
  ...
  write(0,100,10,0,"Press (Space) to change Property");
  write(0,100,20,0,"Photon. (Room Props Example)  ");
  
  write(0,50,20,0,"FPS: ");  
  write_int(0,100,20,0, &fps);
  write_var(0,10,30,0, mensaje_evento);
  ...
  while (!key(_esc) )   
	...
	if (PHOTON_CONNECTED)
   	  ...
   	  if (  key(_space) )	   
	   res= PHOTON_ROOM_SET_PROPS("mapaX", itoa(mapaX))  ;
	   mapaX +=100;
...
begin
   ...
   loop     
	  ...
	  while (   (  evento_red = PHOTON_EVENT_GET()  )     > 0)
         ...
         elseif (evento == PHOTON_EVENT_ROOM_PROPERTIES_CHANGE )
		     mensaje_evento ="ROOM PROPERTIES CHANGE (mapaX) : " +	
         	PHOTON_ROOM_GET_PROPS( "mapaX");	
