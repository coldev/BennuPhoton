...
global
   ...
   string MatchName= "My_PlayerKick_Gemix"; 
...
Begin
 ...
 //==========================================  
  ...
  write(0,100,10,0,"Gemix Photon. (Player Kick Example) ");
   ...
   loop     
	  ...
	  while (   (  evento_red = PHOTON_EVENT_GET()  )     > 0)
		 ...
		 elseif (evento == PHOTON_EVENT_JOINED )
			 ...
			 create_player_kick_event();//only available in join lobby event
		 ...
		 elseif (evento == PHOTON_EVENT_PLAYER_KICKED )
		     mensaje_evento ="event player kicked";	    	    
			 say("player kicked.. ");
...
process create_player_kick_event()
private 
   string mensajeconectado, titulo;
...
begin  
  ...
  loop 
     if (key(_k) AND photon_is_master_client)	   
	     photon_player_kick(2 );    //only KICK player_id = 2 in this example
     end 	

     if ( photon_is_master_client)	   
	     titulo = " Press (K) to kick player2";
	 else 
      	 titulo = "";
     end 	
