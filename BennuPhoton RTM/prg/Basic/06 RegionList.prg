...
global
   ...
   string MatchName= "My_Region_List"; 
...
Begin
  ...
  //selecciona region  y activa modo seleccion/listado de region 
  //mas codigos de regiones en : 
 ...
 //==========================================  
  ...
  write(0,100,10,0,"  Photon. (Region List Example) ");
  
  write(0,50,20,0,"FPS: ");  
  write_int(0,100,20,0, &fps);
  write(0,100,30,0, mensaje_evento);
   ...
   loop     
	  ...
	  while (   (  evento_red = PHOTON_EVENT_GET()  )     > 0)
		 ...
		 elseif (evento == PHOTON_EVENT_JOINED )
			 ...
			 create_region_list_event();//only available in join lobby event
...
process create_region_list_event()
private
  int   i, totalregions;  
  string regionname;
...
begin
  ...
  totalregions= photon_region_list( ); //get  regions
  
  write(0,50,60,0, "Total Regions: "+itoa(totalregions) );
  
  for(i=0;i<totalregions;i++)
   regionname = photon_region_get(i);   
      
   write(0,50,70+(i*10) ,0,
	 "Code/Token: " +regionname 	  
	 );     
  end	
