...
tamano_posicion2D = sizeof(Posicion2D);
  ...
  write(0,100,20,0,"Photon. (Props Example)  ");
  ...
  while (!key(_esc) )   
	...
	if (PHOTON_CONNECTED)
   	  ...
   	  if (  key(_space) )	   
	   PHOTON_PLAYER_SET_PROPS("score", itoa(score));
	   score +=10;
...
begin
   ...
   loop     
	  ...
	  while (   (  evento_red = PHOTON_EVENT_GET()  )     > 0)
         ...
         elseif (evento == PHOTON_EVENT_PLAYER_PROPERTIES_CHANGE )
		     mensaje_evento ="PLAYER PROPERTIES CHANGE (score) : " +	
         	PHOTON_PLAYER_GET_PROPS(codigo, "score");	
